...
static motor_measure_t motor_yaw, motor_pit;
cap_measure_t get_cap;
referee_measure_t get_referee;
kind_measure_t kind;
...
//        get_cap.current=(fp32)(((int32_t)(rx_data[3]) << 8) |(int32_t)(rx_data[2]))/100;       //电容电压
        
        get_cap.power=(fp32)get_cap.invot*get_cap.current; 				//设定功率
		  get_cap.invot =   (fp32)pPowerdata[0]/100.f;//(fp32)(((int32_t)(rx_data[1]) << 8 )| (int32_t)(rx_data[0]))/100;        //输入电压       
        get_cap.capvot = (fp32)pPowerdata[1]/100.f; //(fp32)(((int32_t)(rx_data[3]) << 8) |(int32_t)(rx_data[2]))/100;       //电容电压
        get_cap.current = (fp32)pPowerdata[2]/100.f; //(fp32)((rx_data[5]) << 8 | (rx_data[4]));      //输入电流
        get_cap.power = (fp32)pPowerdata[3]/100.f;//get_cap.invot*get_cap.capvot;//(fp32)((rx_data[7]) << 8 | (rx_data[6])); 
        break;
      }
...
/**
  ...
  * @param[in]      rev: (0x207) reserve motor control current
  * @param[in]      rev: (0x208) reserve motor control current
  * @retval         none
  */
/**
  * @brief          发送电机当前状态(0x205,0x206,0x207,0x208)
...
void CAN_CMD_cap(int16_t temPower)
{
    ...
    capid_can_send_data[1] = temPower;      //功率控制数据45W~100W，4500~10000
...
/**
  * @brief          return the shoot 3508 motor data point
  * @param[in]      i: motor number,range [0,3]
  * @retval         motor data point
  */
/**
  * @brief          返回射击 3508电机数据指针
